...
check-wasm: ## Fail if WASM contains DB driver / DSN code
	...
	@if strings web/public-pages/wasm/erdlens.wasm | grep -Eiq 'jackc/pgx|pgx/v5|postgres://|pgxpool|database/sql|go-sql-driver/mysql|modernc.org/sqlite|microsoft/go-mssqldb|mysql://|mariadb://|sqlite://|sqlite3://|mssql://|sqlserver://'; then \
		echo "❌ WASM contains database connection symbols — keep cmd/erdlens-wasm free of introspect/drivers"; \
		strings web/public-pages/wasm/erdlens.wasm | grep -Ei 'jackc/pgx|pgx/v5|postgres://|pgxpool|database/sql|go-sql-driver/mysql|modernc.org/sqlite|microsoft/go-mssqldb|mysql://|mariadb://|sqlite://|sqlite3://|mssql://|sqlserver://' | head -20; \
		exit 1; \
	else \
